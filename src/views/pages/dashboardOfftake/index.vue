...
const datalist = ref([
  {
    sku: {
      name: "เอนฟาแล็ค เอพลัส ทูแอฟแอล สูตร2 1425กรัม",
      picture: [{
        name: "2024-11-15-123312-Ellipse 200.jpg",
        url: '',      
      }],
    },
  },
  ...
  >
            ...
            <VRow>
              ...
              >
                <AppAutocomplete 
                  ...
                  clearable 
                  clear-icon="tabler-x"
                  @update:model-value="get_all_filters_Brands"
                />
              </VCol>
              <VCol
                cols="12"
              ...
              >
                <AppAutocomplete 
                  ...
                  clearable 
                  clear-icon="tabler-x"
                  @update:model-value="get_all_filters_subBrands"
                />
              </VCol>
              <VCol
                cols="12"
              ...
              >
                <AppAutocomplete 
                  ...
                  clearable 
                  clear-icon="tabler-x"
                  @update:model-value="get_all_filters_products"
                />
              </VCol>
              <VCol
                cols="12"
    ...
    <VCardText>
      <VRow>
              ...
              >
                ...
                <div>
                  <VueApexCharts
                    ...
                    :series="account.brands.map(b => parseInt(b.total_amount))"
                  />
                </div>
              </VCol>
            </VRow>
        ...
        >
          ...
          >
            <VCard>
              <VCardItem title="Offtake Top 5" />
              <VCardText>
                <VList class="card-list">
                  <VListItem
                    v-for="(course, index) in top5_offtake_products"
                    :key="index"
                  >
                    <template #prepend>
                      <VAvatar
                      ...
                      >
                        {{ formatNumber(course.total_amount) }}
                      </VAvatar>
                    </template>
          ...
          >
            <VCard>
              <VCardItem title="Offtake Less 5" />
              <VCardText>
                <VList class="card-list">
                  <VListItem
                    v-for="(course, index) in less5_offtake_products"
                    :key="index"
                  >
                    <template #prepend>
                      <VAvatar
    ...
    <VCardText>
      <VCard>
        <VCardText>
          <VTabs
            v-model="currentTabareaManagers"
            align-tabs="center"
          >
            <VTab
              v-for="(value,index) in areaManagersList"
              :key="index"
              @click="areaSupervisor(value.id)"
            >
              {{ value.name }}
            </VTab>
          </VTabs>
          ...
          <VCardText style="padding-block: 5px;padding-inline: 0;">
            <VWindow v-model="currentTabareaManagers">
              ...
              >
                <VTabs
                  v-model="currentTab2areaSupervisor"
                  align-tabs="center"
                >
                  <VTab
                    v-for="(value2,index2) in areaSupervisorList"
                    :key="index2"
                    @click="getBrandsOfftake(value2.id)"
                  >
                    {{ value2.name }}
                  </VTab>
                </VTabs>
                ...
                <VCardText style="padding-block: 5px;padding-inline: 0;">
                  <VWindow v-model="currentTab2areaSupervisor">
                    ...
                    >
                      <VTabs
                        v-model="currentTab3brand"
                        align-tabs="center"
                      >
                        <VTab
                          v-for="(value3,index3) in brandList"
                          :key="index3"
                          @click="selectBrand(value3.id)"
                        >
                          {{ value3.name }}
                        </VTab>
                      </VTabs>
                          ...
                          <VWindowItem>
                            ...
                            <div>
                              <div style="display: flex;align-items: center;">
                                ...
                                <div
                                  v-for="(value3xxx,index3xxx) in accountDataNew"
                                  :key="index3xxx"
                                  style="display: flex;align-items: center;padding: 12px;inline-size: 150px;"
                                >
                              ...
                              <div
                                v-for="(value4,index4) in groupedDataNew"
                                :key="index4"
                              >
                                <div style="display: flex;align-items: center;">
                                  <div style="display: flex;align-items: center;padding: 12px;inline-size: 400px;">
                                  ...
                                  <div
                                    v-for="(value4x,index4x) in value4.accounts"
                                    :key="index4x"
                                    style="display: flex;align-items: center;padding: 12px;inline-size: 150px;"
                                  >
                                    {{ value4x.amount }}
  ...
  methods: {
    ...
    getPieChartConfig(brands) {
      ...
      return {
        ...
        colors: ["#ff5733", "#33ff57", "#3357ff", "#ff33a1", "#f4c542"],
    ...
    async dashboard_offtake() {
      ...
      try {
        ...
        const response = await apiService.dashboard_offtake(form)

        console.log(response.data.status)
        if (response.data.status === "success") {
          this.stockData = response.data.data
          console.log(this.stockData)
          this.top5_offtake_products = response.data.top5_offtake_products
          this.less5_offtake_products = response.data.less5_offtake_products
          vm.areaManagers()
          vm.getTableProductOfftake()
        }          
      } catch (error) {
    ...
    async dashboard_offtake_table() {
      ...
      try {
        ...
        const response = await apiService.dashboard_offtake_table(form)
        if (response.data.status === "success") {
    ...
    async areaSupervisor(id) {
      ...
      try {
        ...
        if(vm.areaSupervisorList && vm.areaSupervisorList.length > 0){
          ...
          vm.getBrandsOfftake(vm.areaSupervisorList[0].id)
        }else{
          this.groupedDataNew = []
          this.accountDataNew = []
        }
    ...
    async getBrandsOfftake(id) {
      ...
      try {
        ...
        const response = await apiService.getBrandsOfftake(form)
    ...
    async selectBrand(id) {
      this.tab_brand = id
      this.getTableProductOfftake()
    },
    async getTableProductOfftake() {
      const vm = this
      try {
        const form = {
        ...
        const response = await apiService.getTableProductOfftake(form)
        if (response.data.status === "success") {
