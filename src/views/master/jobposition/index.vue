...
const update_isActive = async v => {
  try {
    await apiService.update_job_position_isActive(v.id, {isActive: v.Active ? "Y" : "N"});
    Swal.fire({
  ...
  try {
    ...
    if (isValid.valid) {
      ...
      if(data_id.value != ""){
          ...
          await apiService.update_job_position(data_id.value, {name: data_name.value});
      } else {
        
          await apiService.create_job_position({name: data_name.value,group_customer_id: userData.value.group_customer_id});
      }
...
const exportToExcel = () => {
  ...
  XLSX.writeFile(workbook, 'Master Job Position.xlsx');
};
