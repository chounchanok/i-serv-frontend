...
const update_isActive = async v => {
  try {
    ...
    await apiService.update_group_customer_isActive(v.id, {isActive: v.Active ? "Y" : "N"})
  ...
  try {
    ...
    if (isValid.valid) {
      ...
      if(data_id.value != ""){
          ...
          await apiService.update_group_customer(data_id.value, {name: data_name.value})
      } else {
          
          await apiService.create_group_customer({name: data_name.value,group_customer_id: userData.value.group_customer_id})
      }
...
const exportToExcel = () => {
  ...
  XLSX.writeFile(workbook, 'Master Group Customer.xlsx');
};
