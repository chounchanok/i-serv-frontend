...
const update_sub_category_isActive = async v => {
  try {
    
    await apiService.update_subcategory_isActive(v.id, {isActive: v.Active ? "Y" : "N"})
    Swal.fire({
  ...
  try {
    ...
    if (isValid.valid) {
      ...
      if(data_id.value != ""){
          ...
          await apiService.update_sub_category(data_id.value, {
              category_id: category_id.value,
              name: data_name.value,
              description: data_description.value,
            })
      } else {
         
          await apiService.create_sub_category({
              category_id: category_id.value,
              name: data_name.value,
              description: data_description.value,
              group_customer_id: userData.value.group_customer_id
      ...
      get_all_sub_category(); // โหลดข้อมูลใหม่
    }
  } catch (err) {
    console.error(err)
  }
...
const exportToExcel = () => {
  ...
  XLSX.writeFile(workbook, 'Master Sub Category.xlsx');
};
  ...
  <VCard
    ...
    >
      ...
      <template #item.isActive="{ item }">
        <VSwitch
          v-model="item.Active"
          @change="update_sub_category_isActive(item)"
        />
      </template>
