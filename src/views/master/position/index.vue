...
const permission_name = ref(null)
const permissionlist = ref([
  { id: 1, name: 'Admin' },
  { id: 2, name: 'Management' },
  { id: 3, name: 'Assistant Management' },
...
const submitFile = async () => {
  ...
  if (!selectedFile.value) {
    ...
    loading.value = true
    await apiService.import_position(formData)
      .then(response => {
        Swal.fire({
...
const getall = async () => {
  ...
  const datalistData = await apiService.get_all_position(form);
...
const update_position_isActive = async v => {
  let body = {
    isActive: v.Active ? "Y" : "N",
  }
  const data = await apiService.update_position_isActive(v.id,body);
  Swal.fire({
...
const editdata = async val => {
  ...
  permission_name.value = val.permission_name;
}
...
const save = async () => {
  ...
  if (isValid.valid) {
    ...
    if(data_id.value != ""){
      const data = await apiService.update_position(data_id.value,body);
    } else {
      const data = await apiService.create_position(body);
    }
...
const exportToExcel = () => {
  ...
  const data = filtereddatalist.value.map(item => ({
    ...
    'ประเภทสิทธิใช้งาน': item.permission_name,
    'ชื่อตำแหน่ง': item.name,
  ...
  XLSX.writeFile(workbook, 'Master Position.xlsx');
};
  ...
  <VCard
    ...
    >
      ...
      <template #item.isActive="{ item }">
        <VSwitch
          v-model="item.Active"
          @change="update_position_isActive(item)"
        />
      </template>
          ...
          <VRow>
            <VCol cols="6" sm="6" md="6">
              <div style="border: 2px dashed #ccc; height: 150px; display: flex; justify-content: center; align-items: center;">
                <VBtn variant="tonal" color="secondary"  class="mr-2" :href="'/file_upload/position.xlsx'" download> 
                  Download Template
                </VBtn>
              </div>
            </VCol>
