...
const submitFile = async () => {
  ...
  if (!selectedFile.value) {
    ...
    loading.value = true
    await apiService.import_posm(formData)
      .then(response => {
        Swal.fire({
...
const getall = async () => {
  ...
  const datalistData = await apiService.get_all_posm(form);
...
const update_posm_isActive = async v => {
  try {
    
    await apiService.update_posm_isActive(v.id, {isActive: v.Active ? "Y" : "N"})
    Swal.fire({
...
const save = async () => {
  ...
  if (isValid.valid) {
    ...
    if(data_id.value != ""){
      const data = await apiService.update_posm(data_id.value,body);
    } else {
      const data = await apiService.create_posm(body);
    }
...
const exportToExcel = () => {
  ...
  XLSX.writeFile(workbook, 'Master POSM.xlsx');
};
  ...
  <VCard
    ...
    >
      ...
      <template #item.isActive="{ item }">
        <VSwitch
          v-model="item.Active"
          @change="update_posm_isActive(item)"
        />
      </template>
          ...
          <VRow>
            <VCol cols="6" sm="6" md="6">
              <div style="border: 2px dashed #ccc; height: 150px; display: flex; justify-content: center; align-items: center;">
                <VBtn variant="tonal" color="secondary"  class="mr-2" :href="'/file_upload/posm.xlsx'" download> 
                  Download Template
                </VBtn>
              </div>
            </VCol>
