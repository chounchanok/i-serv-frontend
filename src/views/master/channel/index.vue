...
// } = await $api("https://api-test.iservreport.com/api/backend/get_all_channel", {
//     method: "GET",
// });
...
const exportToExcel = () => {
  ...
  XLSX.writeFile(workbook, 'Master Channel.xlsx');
};
const update_isActive = async v => {
  try {
    
    await apiService.update_channel_isActive(v.id, {isActive: v.Active ? "Y" : "N"})
  ...
  try {
    ...
    if (isValid.valid) {
      ...
      if(data_id.value != ""){
          ...
          await apiService.update_channel(data_id.value, {name: data_name.value,group_customer_id: group_customer_id.value})

      } else {
         
          await apiService.create_channel({name: data_name.value,group_customer_id: group_customer_id.value})
      }
