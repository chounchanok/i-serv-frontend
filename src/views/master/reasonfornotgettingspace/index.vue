...
const submitFile = async () => {
  ...
  if (!selectedFile.value) {
    ...
    loading.value = true
    await apiService.import_reasonfornotgettingspace(formData)
      .then(response => {
        Swal.fire({
...
const getall = async () => {
  ...
  const datalistData = await apiService.get_all_reason(form);
...
const update_reason_isActive = async v => {
  let body = {
    isActive: v.Active ? "Y" : "N",
  }
  const data = await apiService.update_reason_isActive(v.id,body);
  Swal.fire({
...
const save = async () => {
  ...
  if (isValid.valid) {
    ...
    if(data_id.value != ""){
      const data = await apiService.update_reason(data_id.value,body);
    } else {
      const data = await apiService.create_reason(body);
    }
...
const exportToExcel = () => {
  ...
  XLSX.writeFile(workbook, 'Master Reason for not getting space.xlsx');
};
  ...
  <VCard
    ...
    >
      ...
      <template #item.isActive="{ item }">
        <VSwitch
          v-model="item.Active"
          @change="update_reason_isActive(item)"
        />
      </template>
          ...
          <VRow>
            <VCol cols="6" sm="6" md="6">
              <div style="border: 2px dashed #ccc; height: 150px; display: flex; justify-content: center; align-items: center;">
                <VBtn variant="tonal" color="secondary"  class="mr-2" :href="'/file_upload/reasonfornotgettingspace.xlsx'" download> 
                  Download Template
                </VBtn>
              </div>
            </VCol>
