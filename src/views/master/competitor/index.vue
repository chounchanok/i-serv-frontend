...
const submitFile = async () => {
  ...
  if (!selectedFile.value) {
    ...
    Swal.fire({
      ...
      if (result.dismiss === Swal.DismissCompetitor.timer) {
        console.log("I was closed by the timer");
      }
    });

    loading.value = true
    await apiService.import_Competitor(formData)
      .then(response => {
        Swal.fire({
...
const getall = async () => {
  ...
  const datalistData = await apiService.get_all_Competitor(form);
...
const exportToExcel = () => {
  ...
  XLSX.writeFile(workbook, 'Master Competitor.xlsx');
};
const update_Competitor_isActive = async v => {
  let body = {
    isActive: v.Active ? "Y" : "N",
  }
  const data = await apiService.update_Competitor_isActive(v.id,body);
  Swal.fire({
...
const save = async () => {
  ...
  if (isValid.valid) {
    ...
    if(data_id.value != ""){
      const data = await apiService.update_Competitor(data_id.value,body);
    } else {
      const data = await apiService.create_Competitor(body);
    }
  ...
  <VCard
    ...
    >
      ...
      <template #item.isActive="{ item }">
        <VSwitch
          v-model="item.Active"
          @change="update_Competitor_isActive(item)"
        />
      </template>
          ...
          <VRow>
            <VCol cols="6" sm="6" md="6">
              <div style="border: 2px dashed #ccc; height: 150px; display: flex; justify-content: center; align-items: center;">
                <VBtn variant="tonal" color="secondary"  class="mr-2" :href="'/file_upload/competitor.xlsx'" download> 
                  Download Template
                </VBtn>
              </div>
            </VCol>
